...
{
    word16_to_bytes aux;
    Serial.println("getStatus: before requestFrom()");
    Wire.requestFrom(this->deviceAddress, 2);
    Serial.println("getStatus: after requestFrom()");
    delayMicroseconds(300);
    // while (Wire.available() < 2) ; // It did not work on Attiny Core
     Serial.println("getStatus: before Wire.read()");
    aux.refined.highByte = Wire.read();
     Serial.println("getStatus: after highByte Wire.read()");
    aux.refined.lowByte = Wire.read();
     Serial.println("getStatus: after lowByte Wire.read()");
    shadowRegisters[0x0A] = aux.raw;
    Serial.println("getStatus: finished");
}
...
{
    Serial.println("waitAndFinishTune: started");
    do
    {
        Serial.println("waitAndFinishTune: before getStatus()");
        getStatus();
        Serial.println("waitAndFinishTune: after getStatus()");
    } while (reg0a->refined.STC == 0);
    Serial.println("waitAndFinishTune: STC == 1");

    getAllRegisters();
     Serial.println("waitAndFinishTune: after getAllRegisters()");
    reg02->refined.SEEK = 0;
    reg03->refined.TUNE = 0;
     Serial.println("waitAndFinishTune: before setAllRegisters()");
    setAllRegisters();
     Serial.println("waitAndFinishTune: after setAllRegisters()");
    do
    {
          Serial.println("waitAndFinishTune: before getStatus()");
        getStatus();
    ...
    Serial.println("waitAndFinishTune: STC == 0");
    Serial.println("waitAndFinishTune: finished");
}
...
{
    Serial.print("setChannel: channel = ");
    Serial.println(channel);
    reg03->refined.CHAN = channel;
    reg03->refined.TUNE = 1;
    Serial.println("setChannel: before setAllRegisters()");
    setAllRegisters();
    Serial.println("setChannel: after setAllRegisters()");
    delayMicroseconds(60000);
    Serial.println("setChannel: before waitAndFinishTune()");
    waitAndFinishTune();
     Serial.println("setChannel: after waitAndFinishTune()");
    Serial.println("setChannel: finished");
}
...
{
    Serial.print("setFrequency: frequency = ");
    Serial.println(frequency);
    uint16_t channel;
    Serial.print("setFrequency: currentFMBand = ");
    Serial.println(this->currentFMBand);
     Serial.print("setFrequency: startBand[this->currentFMBand] = ");
    Serial.println(this->startBand[this->currentFMBand]);
     Serial.print("setFrequency: currentFMSpace = ");
    Serial.println(this->currentFMSpace);
    Serial.print("setFrequency: fmSpace[this->currentFMSpace] = ");
    Serial.println(this->fmSpace[this->currentFMSpace]);
    channel = (frequency - this->startBand[this->currentFMBand]) / this->fmSpace[this->currentFMSpace];
    Serial.print("setFrequency: Calculated Channel = ");
    Serial.println(channel);
    setChannel(channel);
    this->currentFrequency = frequency;
     Serial.println("setFrequency: finished");
}
